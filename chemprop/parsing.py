...
def add_train_args(parser: ArgumentParser):
    ...
    parser.add_argument('--checkpoint_dir', type=str, default=None,
                        help='Directory from which to load model checkpoints'
                             '(walks directory and ensembles all models that are found)')
    parser.add_argument('--checkpoint_dir_2', type=str, default=None,
                        help='Directory from which to load model checkpoints'
                             '(walks directory and ensembles all models that are found)')
    parser.add_argument('--checkpoint_dir_3', type=str, default=None,
                        help='Directory from which to load model checkpoints'
                             '(walks directory and ensembles all models that are found)')

    parser.add_argument('--checkpoint_path', type=str, default=None,
                        help='Path to model checkpoint (.pt file)')
    parser.add_argument('--checkpoint_path_2', type=str, default=None,
                        help='Path to model checkpoint (.pt file)')
    parser.add_argument('--checkpoint_path_3', type=str, default=None,
                        help='Path to model checkpoint (.pt file)')
    ...
    parser.add_argument('--split_sizes', type=float, nargs=3, default=[0.8, 0.2, 0],
    ...
    parser.add_argument('--metric', type=str, default='mse', #none
    ...
    parser.add_argument('--ensemble_size_2', type=int, default=1,
                        help='Number of models in ensemble')
    parser.add_argument('--ensemble_size_3', type=int, default=1,
                        help='Number of models in ensemble')
    ...
    parser.add_argument('--tr_head_size', type=int, default=32)  #32
...
def update_checkpoint_args_2(args: Namespace):
    """
    Walks the checkpoint directory to find all checkpoints, updating args.checkpoint_paths and args.ensemble_size.

    :param args: Arguments.
    """
    if hasattr(args, 'checkpoint_paths_2') and args.checkpoint_paths_2 is not None:
        return

    if args.checkpoint_dir_2 is not None and args.checkpoint_path_2 is not None:
        raise ValueError('Only one of checkpoint_dir and checkpoint_path can be specified.')

    if args.checkpoint_dir_2 is None:
        args.checkpoint_paths_2 = [args.checkpoint_path_2] if args.checkpoint_path_2 is not None else None
        return

    args.checkpoint_paths_2 = []

    for root, _, files in os.walk(args.checkpoint_dir_2):
        for fname in files:
            if fname.endswith('.pt'):
                args.checkpoint_paths_2.append(os.path.join(root, fname))

    args.ensemble_size_2 = len(args.checkpoint_paths_2)

    if args.ensemble_size_2 == 0:
        raise ValueError(f'Failed to find any model checkpoints in directory "{args.checkpoint_dir_2}"')

def update_checkpoint_args_3(args: Namespace):
    """
    Walks the checkpoint directory to find all checkpoints, updating args.checkpoint_paths and args.ensemble_size.

    :param args: Arguments.
    """
    if hasattr(args, 'checkpoint_paths_3') and args.checkpoint_paths_3 is not None:
        return

    if args.checkpoint_dir_3 is not None and args.checkpoint_path_3 is not None:
        raise ValueError('Only one of checkpoint_dir and checkpoint_path can be specified.')

    if args.checkpoint_dir_3 is None:
        args.checkpoint_paths_3 = [args.checkpoint_path_3] if args.checkpoint_path_3 is not None else None
        return

    args.checkpoint_paths_3 = []

    for root, _, files in os.walk(args.checkpoint_dir_3):
        for fname in files:
            if fname.endswith('.pt'):
                args.checkpoint_paths_3.append(os.path.join(root, fname))

    args.ensemble_size_3 = len(args.checkpoint_paths_3)

    if args.ensemble_size_3 == 0:
        raise ValueError(f'Failed to find any model checkpoints in directory "{args.checkpoint_dir_3}"')
...
def modify_train_args(args: Namespace):
    ...
    update_checkpoint_args(args)
    update_checkpoint_args_2(args)
    update_checkpoint_args_3(args)
