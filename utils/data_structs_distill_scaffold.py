...
class MolData(Dataset):
    ...
    def __init__(self, train_data_scaffold,train_data_decorator, voc):
        self.voc = voc
        # self.smiles = []
        df = pd.read_csv(train_data_scaffold)
        df.columns = ['Scaffold']
        self.scaffold = df['Scaffold'].values.tolist()

        df_dec = pd.read_csv(train_data_decorator)
        df_dec.columns = ['Decorator']
        self.decorator = df_dec['Decorator'].values.tolist()
