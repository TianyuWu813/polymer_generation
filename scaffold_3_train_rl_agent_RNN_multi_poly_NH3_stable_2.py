...
from utils.utils import Variable, seq_to_smiles, fraction_valid_smiles, unique, fraction_valid_smiles_scaffold,fraction_valid_smiles_scaffold_ring
...
def train_agent_func_test_args(arg_dict, input_scaffold_path):
    ...
    optimizer = torch.optim.Adam(Agent.decorator.parameters(), lr=0.0001)
    ...
    for step in range(n_steps):
        ...
        for step_, batch in tqdm(enumerate(generate_data), total=len(generate_data)):
            ...
            for scaffold_1, scaffold_2, count_1, count_2, decorator, attach_points, end_scaffold_1, end_scaffold_2 in zip(
                ...
                # print(decorator)
                # if ('N' in decorator) or ('n'in decorator):
            ...
            for poly_1, poly_2 in zip(polymer_1_unit, polymer_2_unit):
                # print(poly_1,poly_2)
                if poly_1 == None or poly_2 == None or (poly_1 == poly_2 == None):
            ...
            for bigsmile, feature, array,atom_val in zip(bigsmiles_list, feature_list, array_featue_list,atom_value_list):
                # print(bigsmile[0])
                if bigsmile[0] == None or feature == None :
                    # print(bigsmile[0], count)
                    scores.append(-5)
                    scores_HC10.append(-5)
                    scores_atom.append(-5)
                    activity.append(-100)
                    count += 1
                else:
                    # print(bigsmile[0], count)
                    ...
                    if value < -1:
                        ...
                        HC10 = 1.573 * np.exp(0.6923 * value)
                    # scores.append(5*sim_val)
                    scores_HC10.append(score_HC10[0])
                    # scores.append(3*score_HC10[0] + ring_val)
                    scores.append(3*score_HC10[0]+atom_val)
            ...
            # itemindex = list(np.where(sum_score == 4))
            ...
            if step % 15 == 0:
                ...
                # plt.title('Fintuning Process for Model Pre-trained by ChEMBL')
                plt.title('Fintuning Process for Model Pre-trained by Graph Grammar Distillation')
                plt.savefig('./MCMG_results_2/' + str(step) + '_reward.svg',dpi=600,format="svg")
                plt.savefig('./MCMG_results_2/' + str(step) + '_reward',dpi=600)
                ...
                if step >400:
                    ...
                    HC10_list = pd.DataFrame(plot_HC10, columns=name_2)
                    atom_list = pd.DataFrame(plot_atom, columns=name_3)
                    reward_list.to_csv('./MCMG_results_2/reward_list.csv', header=True, index=True)
                    HC10_list.to_csv('./MCMG_results_2/HC10_list.csv', header=True, index=True)
                    atom_list.to_csv('./MCMG_results_2/atom_list.csv', header=True, index=True)
...
if __name__ == "__main__":
    ...
    input_scaffold_path = './data/target_scaffold_all.csv'  #6_HUAN
    ...
    parser.add_argument('--middle', action='store', dest='restore_prior_from',
                        ...
                        default='./ckpt/CNO_scaffold_middle_lr1e-9_NH3_7.19_best.ckpt',
    ...
    parser.add_argument('--save-file-path', action='store', dest='agent_save', default='./data/RL_agent_2.ckpt',
    ...
    # arg_dict.checkpoint_dir = './ckpt/HC10_all_feature_only/choose_4'
    arg_dict.checkpoint_dir = 'D:\\Programme\\CMPNNBigsmile\\ckpt\\result_summary\\RFE\\HC10(SMILES+FEATURE)\\fold_13'
