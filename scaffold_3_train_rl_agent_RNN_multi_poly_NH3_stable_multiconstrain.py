...
from chemprop.train.make_predictions import make_predictions_scaffold,make_predictions_scaffold_2,make_predictions_scaffold_3
        ...
        for step_, batch in tqdm(enumerate(generate_data), total=len(generate_data)):
            ...
            for bigsmile, feature, array,sim_val,ring_val,atom_val in zip(bigsmiles_list, feature_list, array_featue_list,sim_value_list,ring_value_list,atom_value_list):
                ...
                if bigsmile[0] == None or feature == None :
                    ...
                    pred_Saureus,smiles_Saureus,feature_Saureus = make_predictions_scaffold_2(args=arg_dict,smiles=bigsmile,features=feature_Saureus,array=array_Saureus)
                    pred_Ecoli,smiles_Ecoli,feature_Ecoli = make_predictions_scaffold_3(args=arg_dict,smiles=bigsmile,features=feature_Ecoli,array=array_Ecoli)

                    # print(pred, 333)
                    # print(pred, 333)
                    # value = pred.sum()/len(pred)
                    value = pred.max()
                    value_HC10 = pred_HC10.max()#pred_HC10.sum()/len(pred_HC10)
                    value_Saureus =pred_Saureus.min()# pred_Saureus.sum()/len(pred_Saureus)
                    value_Ecoli = pred_Ecoli.min()#pred_Ecoli.sum()/len(pred_Ecoli)
                    # print(value,count)
                    ...
                    if value < -1:
                        ...
                        HC10 = 1.573 * np.exp(0.6923 * value)
                    # scores.append(5 * score_HC10[0] + 5*sim_val)
            ...
            if step % 15 == 0:
                ...
                plt.savefig('./MCMG_results/' + str(step) + '_reward',dpi=300)
                plt.draw()
...
if __name__ == "__main__":
    ...
    input_scaffold_path = './data/target_scaffold_all.csv'
