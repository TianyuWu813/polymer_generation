...
class GraphFormer(pl.LightningModule):
    ...
    def forward(self, batched_data, perturb=None):
        ...
        graph_attn_bias = graph_attn_bias.unsqueeze(1).repeat(1, self.head_size, 1,
                                                              1)  # [n_graph, n_head, n_node+1, n_node+1]

        # rel pos
        rel_pos_bias = self.rel_pos_encoder(rel_pos).permute(0, 3, 1,
                                                             2)  # [n_graph, n_node, n_node, n_head] -> [n_graph, n_head, n_node, n_node]
        ...
        if self.edge_type == 'multi_hop':
            ...
            edge_input_flat = torch.bmm(edge_input_flat,
                                        self.edge_dis_encoder.weight.reshape(-1, self.head_size, self.head_size)[
                                        :max_dist, :, :])
            edge_input = edge_input_flat.reshape(max_dist, n_graph, n_node, n_node, self.head_size).permute(1, 2, 3, 0,
                                                                                                            4)
            edge_input = (edge_input.sum(-2) / (rel_pos_.float().unsqueeze(-1))).permute(0, 3, 1, 2)
        else:
            edge_input = self.edge_encoder(attn_edge_type).mean(-2).permute(0, 3, 1,
                                                                            2)  # [n_graph, n_node, n_node, n_head] -> [n_graph, n_head, n_node, n_node]
    ...
    def training_step(self, batched_data, batch_idx):
        if self.dataset_name == 'ogbg-molpcba':
            ...
            if not self.flag:
                ...
                optimizer = self.optimizers()
                optimizer.zero_grad()
                loss, _ = flag_bounded(model_forward, perturb_shape, y_gt, optimizer, batched_data.x.device,
                                       self.loss_fn,
