...
import utils.scaffold_utils.scaffold_ring as uscr
...
def fraction_valid_smiles_scaffold(scaffold, decorator):
    """Takes a list of SMILES and returns fraction valid."""
    i = 0
    for sca,dec in zip(scaffold, decorator):
        if usc.join_joined_attachments(sca, dec):
            i += 1
    return i / len(scaffold)

def fraction_valid_smiles_scaffold_ring(scaffold, decorator):
    """Takes a list of SMILES and returns fraction valid."""
    i = 0
    for sca,dec in zip(scaffold, decorator):
        if uscr.join_joined_attachments(sca, dec):
            i += 1
    return i / len(scaffold)
