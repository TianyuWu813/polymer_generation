...
class StackAugmentedRNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, layer_type='GRU',
                 ...
                 stack_width=None, stack_depth=None):
        """
        ...
        self._attention = AttentionLayer(hidden_size)

        self.linear_pro = nn.Linear(256,hidden_size)
        # self.lr = lr
    ...
    def forward(self, inp, hidden, stack,encoder_padded_seqs):
        """
        ...
        encoder_padded_seqs = self.linear_pro(encoder_padded_seqs)

        output=output.transpose(0,1)


        mask = (output[:, :, 0] != 0).unsqueeze(dim=-1).type(torch.float)
        attn_padded_encoded_seqs, attention_weights = self._attention(output, encoder_padded_seqs, mask)

        attn_padded_encoded_seqs = attn_padded_encoded_seqs.transpose(0, 1).squeeze(0)


        attn_padded_encoded_seqs = self.decoder(attn_padded_encoded_seqs)

        return attn_padded_encoded_seqs, next_hidden, stack
    ...
    def init_hidden(self,batch_size):
        ...
        """
        if self.use_cuda:
            return Variable(torch.zeros(self.n_layers * self.num_dir, batch_size,
