...
def join(scaffold_smi, decoration_smi, keep_label_on_atoms=False):  # huan zhuang lianjie
    """
    ...
    if scaffold and decoration:
        ...
        dec_enc_atom = []
        for i in range(len(decoration.GetAtoms())):
            if (combined_scaffold.GetAtoms()[i].GetDegree() == 1 and combined_scaffold.GetAtoms()[i].GetSymbol() != ATTACHMENT_POINT_TOKEN) or (
                    combined_scaffold.GetAtoms()[i].GetDegree() == 2 and combined_scaffold.GetAtoms()[i].IsInRing()):
                dec_enc_atom.append(combined_scaffold.GetAtoms()[i])
        # print(dec_enc_atom,777)
        ...
        for atom in attachments:
            ...
            if atom.GetDegree() != 1:
                count = True  # the attachment is wrongly generated
                # neighbors.append(atom.GetNeighbors())
                # neighbors.append(atom.GetNeighbors())
            # if atom.GetDegree() == 2:
            #     count = True  # the attachment is wrongly generated
            #     neighbors.append(atom.GetNeighbors())
            if atom.GetDegree() > 2 or atom.GetDegree() == 0:
                return None
            neighbors.append(atom.GetNeighbors())
            # print(atom.GetNeighbors(),11)


        bonds = [atom.GetBonds()[0] for atom in attachments]
        # print(neighbors,555)
        # print(dec_enc_atom,len(dec_enc_atom))
        ...
        if count == True:# and len(dec_enc_atom)>1:
            combined_scaffold.AddBond(neighbors[0][0].GetIdx(), neighbors[1][0].GetIdx(), bond_type)
            combined_scaffold.AddBond(dec_enc_atom[-1].GetIdx(), neighbors[1][1].GetIdx(), bond_type)
            # combined_scaffold.AddBond(neighbors[0][0].GetIdx(), neighbors[1][1].GetIdx(), bond_type)
        elif count == False:
            combined_scaffold.AddBond(neighbors[0][0].GetIdx(), neighbors[1][0].GetIdx(), bond_type)
        combined_scaffold.RemoveAtom(attachments[0].GetIdx())
        combined_scaffold.RemoveAtom(attachments[1].GetIdx())
        # print(rkc.MolToSmiles(combined_scaffold),888)
...
def join_joined_attachments(scaffold_smi, decorations_smi):
    ...
    scaffold_smi = add_attachment_point_numbers_uncan(scaffold_smi)
...
def add_attachment_point_numbers(mol_or_smi, canonicalize=True):  #canonicalize=True
    """
...
def add_attachment_point_numbers_uncan(mol_or_smi, canonicalize=False):  #canonicalize=True
    """
    Adds the numbers for the attachment points throughout the molecule.
    :param mol_or_smi: SMILES string to convert.
    :param canonicalize: Canonicalize the SMILES so that the attachment points are always in the same order.
